...
const Form = ({ type, post, submitting, handleSubmit, setPost }) => {
  return (
    <section className='w-full max-w-full flex-start flex-col'>
      ...
      <p className='desc text-left max-w-md'> {type} and share amazing propmpts with the world, and let your
      ...
      <form onSubmit={handleSubmit} className='mt-10 w-full max-w-2xl flex flex-col gap-7 glassmorphism'>
        ...
        <div className='flex-end mx-3 mb-5 gap-4'>
          ...
          <button type='submit' disabled={submitting} className='px-5 py-1.5 rounded-full text-white bg-primary-orange'>{submitting ? `${type}...` : `${type}`}</button>
        </div>
      </form>
    </section>
  )
